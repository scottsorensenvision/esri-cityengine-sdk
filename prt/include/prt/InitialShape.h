...
public:
	 ...
	 * Faces have counter-clockwise vertex ordering.
	 *
	 ...
	 * /cgaenv/maxDerivationDepth    | int         | 99      | Maximum recursion level for cga operations
	 * /cgaenv/maxDerivationWidth    | int         | 50000   | Maximum shapetree width (for cga)
	 * /cgaenv/maxFuncCallDepth      | int         | 256     | Maximum recursion level for cga functions; int; default: 256
	 ...
	 * @param startRule          The start rule, with style prefix (e.g. "Default$Init").  This will be the @cgaref{attr_initialShape.html,initialShape.startRule cga attribute}.
	 ...
	 * \sa resolveGeometry
	 */
	virtual Status resolveAttributes(	wchar_t const*		key,
										ResolveMap const*	resolveMap	= 0,
										Cache*				cache		= 0
									) = 0;
};
